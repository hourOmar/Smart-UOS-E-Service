...
import { sampleAdminHistory } from '../../../mocks/requests.mock';
...
export const AdminHistoryPage: React.FC<AdminHistoryPageProps> = ({
  ...
  const filteredHistory = sampleAdminHistory.filter((item) => {
    const matchesFilter =
      selectedFilter === 'All' ||
    ...
    const matchesSearch =
      ...
      item.studentName.toLowerCase().includes(effectiveSearch.toLowerCase());

    return matchesFilter && matchesSearch;
  });

  return (
    <div id="admin-history-page" className="p-6 max-w-7xl mx-auto flex flex-col gap-6">
      {/* Header & Local Search */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-[#6B7280] mt-1">
            Archived and processed academic petitions across all departments
          </p>
        </div>
      ...
      {/* Request History Cards Grid (8 Items) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredHistory.map((req, idx) => {
          return (
            <div
              key={idx}
              id={`admin-history-card-${idx}`}
        ...
        <ModalShell>
          ...
          <div className="my-4 flex flex-col gap-2 text-xs">
            ...
            <p className="text-[#6B7280]">
              Student: <strong>{selectedCard.studentName}</strong> ({selectedCard.studentId})
            </p>
            <p className="text-[#6B7280]">
              Department: <strong>{selectedCard.department}</strong>
            </p>
            <div className="p-3 bg-[#F9FAFB] rounded-xl text-[11px] text-[#4B5563]">
              Official Committee Decision logged. All transcripts and records updated in Banner ERP system.
            </div>
          </div>
